...
parser.add_argument('--path_prefix', type=str, default='/path/to/data_fold/')
...
parser.add_argument('--low_percent', type=int, default=10)
parser.add_argument('--train_low', action='store_true')
parser.add_argument('--scheduler', action='store_true')
parser.add_argument('--newest_low', action='store_true')
parser.add_argument('--new_seed', action='store_true')
...
if args.new_seed:
    ...
    val_rand_sampler = RandEdgeSampler(full_data.sources, full_data.destinations, seed=5)
...
for i in range(args.n_runs):
    ...
    tgn = TGN(neighbor_finder=train_ngh_finder, node_features=node_features,
              ...
              time_line_length=args.time_line_length,)
    if args.fine_tune:
        tgn.load_state_dict(torch.load(args.model_pth, map_location=device))
    criterion = torch.nn.BCELoss()
    optimizer = torch.optim.Adam(tgn.parameters(), lr=LEARNING_RATE)
    if args.scheduler:
        logger.info('use lr scheduler')
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda e: 0.9 ** (e // 2))
