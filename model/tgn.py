...
class TGN(torch.nn.Module):
    ...
    def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_nodes, edge_times,
        ...
        if self.use_time_line:
            ...
            prev_neg_timestamps = self.neighbor_finder.get_time_stamps(negative_nodes, edge_times)
            # select time stamps to form a time line

            for i in range(self.time_line_length):
                selected_src_timestamps = []
                selected_dst_timestamps = []
                selected_neg_timestamps = []

                for src_timestamps, dst_timestamps, neg_timestamps in \
                        zip(prev_src_timestamps, prev_dst_timestamps, prev_neg_timestamps):
                    ...
                    selected_neg_timestamps.append(get_previous_timestamp(neg_timestamps, i, n_neighbors))

                nodes = np.concatenate([nodes, source_nodes, destination_nodes, negative_nodes])
                timestamps = np.concatenate(
                    [timestamps, selected_src_timestamps, selected_dst_timestamps, selected_neg_timestamps]
                )
        ...
        if self.use_time_line:
            ...
            for time_line_idx in range(self.time_line_length):
                prev_src_node_embedding = node_embedding[3 * n_samples + 3 * n_samples * time_line_idx:
                                                         4 * n_samples + 3 * n_samples * time_line_idx]
                prev_dst_node_embedding = node_embedding[4 * n_samples + 3 * n_samples * time_line_idx:
                                                         5 * n_samples + 3 * n_samples * time_line_idx]
                prev_neg_node_embedding = node_embedding[5 * n_samples + 3 * n_samples * time_line_idx:
                                                         6 * n_samples + 3 * n_samples * time_line_idx]
                prev_node_embeddings.append(torch.cat([prev_src_node_embedding,
                                                       prev_dst_node_embedding,
                                                       prev_neg_node_embedding]))
            ...
            embeddings_after_time_agg = self.agg_time_line(cur_local_embed=cur_local_embed,
                                                           prev_local_embeds=prev_node_embeddings)
            ...
            negative_node_embedding = embeddings_after_time_agg[2 * n_samples: 3 * n_samples]
